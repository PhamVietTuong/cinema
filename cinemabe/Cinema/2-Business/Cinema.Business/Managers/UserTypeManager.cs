...
public class UserTypeManager : IUserTypeManager
{
        ...
        foreach (var key in filters.Keys)
        {
            ...
            switch (key)
            {
                ...
                case "name":
                    ...
                    query = _uow.UserTypeStore.FilterQuery(query, e => e.Name.Contains(name));
                    break;
            }
        }
        return query;
    ...
    private IQueryable<UserType> ApplySort(IQueryable<UserType> query, SortDTO? sort)
    {
        ...
        {
            "name" => _uow.UserTypeStore.OrderQuery(query, e => e.Name, sort.Ascending),
            _ => query,
        };
    }
