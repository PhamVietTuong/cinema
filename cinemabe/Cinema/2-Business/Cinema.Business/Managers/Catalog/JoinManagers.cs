...
public class SeatTypeTicketTypeManager(IApplicationUnitOfWork uow) : ISeatTypeTicketTypeManager
{
    public async Task<DefaultSearchResults<SeatTypeTicketTypeDTO>> GetAsync(PagingSearchDTO search)
    {
        search ??= new PagingSearchDTO();
        var all = (await uow.SeatTypeTicketTypeStore.GetAllAsync())
            .Select(x => new SeatTypeTicketTypeDTO
            {
                SeatTypeId = x.SeatTypeId,
                TicketTypeId = x.TicketTypeId,
                PriceMultiplier = x.PriceMultiplier,
                SeatTypeName = x.SeatType?.Name ?? string.Empty,
                TicketTypeName = x.TicketType?.Name ?? string.Empty,
            })
            .ToList();
        ...
        return new DefaultSearchResults<SeatTypeTicketTypeDTO>
        {
            Results = paged,
            TotalCount = all.Count,
            CountPerPage = pageSize,
    ...
    public async Task<SeatTypeTicketTypeDTO> CreateAsync(CreateSeatTypeTicketTypeRequest request)
    {
        var existing = await uow.SeatTypeTicketTypeStore.GetAsync(request.SeatTypeId, request.TicketTypeId);
        if (existing != null)
        {
            throw new InvalidOperationException("This seat-type / ticket-type pairing already exists.");
        }
        await uow.SeatTypeTicketTypeStore.AddAsync(new SeatTypeTicketType
        {
            SeatTypeId = request.SeatTypeId,
            TicketTypeId = request.TicketTypeId,
            PriceMultiplier = request.PriceMultiplier,
        });
        return new SeatTypeTicketTypeDTO
        {
            SeatTypeId = request.SeatTypeId,
            TicketTypeId = request.TicketTypeId,
            PriceMultiplier = request.PriceMultiplier,
    ...
    public async Task<SeatTypeTicketTypeDTO> UpdateAsync(UpdateSeatTypeTicketTypeRequest request)
    {
        var entity = await uow.SeatTypeTicketTypeStore.GetAsync(request.SeatTypeId, request.TicketTypeId)
                     ?? throw new KeyNotFoundException("Seat-type / ticket-type pairing not found.");
        entity.PriceMultiplier = request.PriceMultiplier;
        await uow.SeatTypeTicketTypeStore.UpdateAsync(entity);
        return new SeatTypeTicketTypeDTO
        {
    ...
    public Task DeleteAsync(Guid seatTypeId, Guid ticketTypeId)
    {
        return uow.SeatTypeTicketTypeStore.DeleteAsync(seatTypeId, ticketTypeId);
    }
}
