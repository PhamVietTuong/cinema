...
export class UserTypesManagementComponent implements OnInit, OnDestroy {
  items: Dto[] = [];
  totalCount = 0;
  pageIndex = 1;
  pageSize = 10;
  ...
  constructor(
    ...
    this.form = this._fb.group({ name: ['', Validators.required] });
  ...
  load(): void {
    this._svc.getUserTypes(CinemaServiceAgent.PagingSearchDTO.fromJS({
  ...
  confirmDelete(): void {
    ...
    if (id) {
      this._svc.deleteUserType(id).subscribe({
        next: () => this.load(),
